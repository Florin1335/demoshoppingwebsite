...
let mailjetClient = mailjet.connect(process.env.mailjetKey || 'ecc8f1b34b0aac72d80efe7d04359771', process.env.mailjetSecretKey || '188b938d212c6ca8fb9dbf88beb6728d');

auth.get('/', expressAsyncHandler((req, res) => {
...
auth.post('/register', expressAsyncHandler(async (req, res) => {
...
auth.post('/activare_cont', async (req, res) => {
    ...
    if (exista){
        ...
        res.status(500).json('Contul a fost deja activat sau codul de activare nu există.')
});
...
auth.post('/adresa/set', async (req, res) => {
    ...
    if (exista){
            ...
            if (ok === 0)
                res.json('status-eroare-date-eronate');
            else{
                ...
                await exista.save((err) => {
                    ...
                    res.json('status-adresa-salvata');
                });   
            } 
    }else
        res.json('status-eroare-utilizatorul-nu-exista');
});
    ...
    if (exista){
        for (let j=0;j<exista.length;j++){
            ...
            for (let i in exista[j].cos){
                ...
                if (produs)
                    obj[produs.denumire] = exista[j].cos[i];
                else
                    obj[i] = exista[j].cos[i];
            }
